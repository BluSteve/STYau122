...
public class MNDOParamErrorFunctionUnrestricted {
    ...
    public MNDOSolutionUnrestricted soln;
    ...
    public MNDOParamErrorFunctionUnrestricted(MNDOAtom[] atoms, MNDOSolutionUnrestricted soln, double refHeat) {
    ...
    public void AddGeomError() {
        ...
        for (int i = 0; i < expatoms.length; i++) {
            for (int j = 0; j < 3; j++) {
                double d = MNDODerivative.gradientUnrestricted(expatoms, expsoln.alphadensity(), expsoln.betadensity(), i, j);
                sum += d * d;
            }
        }
    ...
    public void AddBondError(int atom1, int atom2, double ref) {
        ...
        double deriv = R[0] / dist * MNDODerivative.gradientUnrestricted(expatoms, expsoln.alphadensity(), expsoln.betadensity(), atom2, 0)
                + R[1] / dist * MNDODerivative.gradientUnrestricted(expatoms, expsoln.alphadensity(), expsoln.betadensity(), atom2, 1)
                + R[2] / dist * MNDODerivative.gradientUnrestricted(expatoms, expsoln.alphadensity(), expsoln.betadensity(), atom2, 2);
