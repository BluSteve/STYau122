        ...
        for (int j = 0; j < orbitals.length; j++) {
            for (int k = j; k < orbitals.length; k++) {
                if (j == k) {
                    for (int l : index[atomNumber[j]]) {
                        if (l > -1) {
                            integralArrayCoulomb[integralCount] = NDDO6G.OneCenterERI(orbitals[j], orbitals[j], orbitals[l], orbitals[l]);
                            integralCount++;
                        }
                    }
                    for (int l : missingIndex[atomNumber[j]]) {
                        if (l > -1) {
                            for (int m : missingIndex[atomNumber[j]]) {
                                if (m > -1) {
                                    if (atomNumber[l] == atomNumber[m]) {
                                        integralArrayCoulomb[integralCount] = NDDO6G.getG(orbitals[j], orbitals[j], orbitals[l], orbitals[m]);
                                        integralCount++;
                                    }
                                }
                            }
                    ...
                    integralArrayCoulomb[integralCount] = 2 * NDDO6G.OneCenterERI(orbitals[j], orbitals[k], orbitals[j], orbitals[k]);
                    integralCount++;
                    for (int l : missingIndex[atomNumber[j]]) {
                        if (l > -1) {
                            for (int m : missingIndex[atomNumber[j]]) {
                                if (m > -1) {
                                    if (atomNumber[l] == atomNumber[m]) {
                                        integralArrayCoulomb[integralCount] = NDDO6G.getG(orbitals[j], orbitals[k], orbitals[l], orbitals[m]);
                                        integralCount++;
                                    }
                                }
                            }
        ...
        for (int j = 0; j < orbitals.length; j++) {
            for (int k = j; k < orbitals.length; k++) {
                ...
                if (j == k) {
                    ...
                    for (int l : index[atomNumber[j]]) {
                        if (l > -1) {
                            integralArrayExchange[integralCount] = -1 * NDDO6G.OneCenterERI(orbitals[j], orbitals[l], orbitals[j], orbitals[l]);
                            integralCount++;
                        }
                    }
                } else if (atomNumber[j] == atomNumber[k]) {
                    //System.err.println ("1.5[" + j + k + "|" + j + k + "] - 0.5[" + j + j + "|" + k + k + "]");
                    integralArrayExchange[integralCount] = -1 * NDDO6G.OneCenterERI(orbitals[j], orbitals[k], orbitals[j], orbitals[k]) - 1 * NDDO6G.OneCenterERI(orbitals[j], orbitals[j], orbitals[k], orbitals[k]);
                    integralCount++;
                } else {
                    for (int l : index[atomNumber[j]]) {
                        if (l > -1) {
                            for (int m : index[atomNumber[k]]) {
                                if (m > -1) {
                                    integralArrayExchange[integralCount] = -1 * NDDO6G.getG(orbitals[j], orbitals[l], orbitals[k], orbitals[m]);
                                    integralCount++;
                                }
                            }
                        }
