...
public class NDDO6G extends STO6G {
	...
	public NDDO6G(NDDOAtom a, OrbitalProperties orbital, double zeta, double beta,
				  double U) {
	...
	public static double betaparamderiv(NDDO6G a, NDDO6G b, int num, int type) {
		return 0.5 * (a.beta + b.beta) * ParamDerivative.getSderivfinite(a, b, num,
				type);
	}
	...
	private static double uzQpipi(double p01, double p11, double p21, double D11,
		...
		return -0.25 * Math.pow((R + D11) * (R + D11) + 4 * D22 * D22 + a12 * a12, -0.5)
				+ 0.25 * Math.pow((R - D11) * (R - D11) + 4 * D22 * D22 + a12 * a12,
				-0.5)
	...
	private static double uzQzz(double p01, double p11, double p21, double D11,
		...
		double a12 = p11 + p22;
		return -0.125 *
				Math.pow((R + D11 - 2 * D22) * (R + D11 - 2 * D22) + a12 * a12, -0.5)
				+ 0.125 *
				Math.pow((R - D11 - 2 * D22) * (R - D11 - 2 * D22) + a12 * a12, -0.5)
				- 0.125 *
				Math.pow((R + D11 + 2 * D22) * (R + D11 + 2 * D22) + a12 * a12, -0.5)
	...
	private static double QpipiQzz(double p01, double p11, double p21, double D11,
		...
		double a22 = p21 + p22;
		return 0.125 *
				Math.pow((R - 2 * D22) * (R - 2 * D22) + 4 * D21 * D21 + a22 * a22, -0.5)
				+ 0.125 *
				Math.pow((R + 2 * D22) * (R + 2 * D22) + 4 * D21 * D21 + a22 * a22, -0.5)
	...
	private static double ssppippi(double p01, double p11, double p21, double D11,
								   double D21, double p02, double p12, double p22,
								   double D12, double D22, double R) {
		return qq(p01, p11, p21, D11, D21, p02, p12, p22, D12, D22, R) +
	...
	private static double sspzpz(double p01, double p11, double p21, double D11,
								 double D21, double p02, double p12, double p22,
								 double D12, double D22, double R) {
		return qq(p01, p11, p21, D11, D21, p02, p12, p22, D12, D22, R) +
	...
	private static double ppippiss(double p01, double p11, double p21, double D11,
								   double D21, double p02, double p12, double p22,
								   double D12, double D22, double R) {
		return qq(p01, p11, p21, D11, D21, p02, p12, p22, D12, D22, R) +
	...
	private static double pzpzss(double p01, double p11, double p21, double D11,
								 double D21, double p02, double p12, double p22,
								 double D12, double D22, double R) {
		return qq(p01, p11, p21, D11, D21, p02, p12, p22, D12, D22, R) +
	...
	private static double ppippippippi(double p01, double p11, double p21, double D11,
									   double D21, double p02, double p12, double p22,
									   double D12, double D22, double R) {
		return qq(p01, p11, p21, D11, D21, p02, p12, p22, D12, D22, R) +
				qQpipi(p01, p11, p21, D11, D21, p02, p12, p22, D12, D22, R) +
	...
	private static double pxpxpypy(double p01, double p11, double p21, double D11,
								   double D21, double p02, double p12, double p22,
								   double D12, double D22, double R) {
		return qq(p01, p11, p21, D11, D21, p02, p12, p22, D12, D22, R) +
				qQpipi(p01, p11, p21, D11, D21, p02, p12, p22, D12, D22, R) +
	...
	private static double ppippipzpz(double p01, double p11, double p21, double D11,
									 double D21, double p02, double p12, double p22,
									 double D12, double D22, double R) {
		return qq(p01, p11, p21, D11, D21, p02, p12, p22, D12, D22, R) +
				qQzz(p01, p11, p21, D11, D21, p02, p12, p22, D12, D22, R) +
	...
	private static double pzpzppippi(double p01, double p11, double p21, double D11,
									 double D21, double p02, double p12, double p22,
									 double D12, double D22, double R) {
		return qq(p01, p11, p21, D11, D21, p02, p12, p22, D12, D22, R) +
				qQpipi(p01, p11, p21, D11, D21, p02, p12, p22, D12, D22, R) +
	...
	private static double pzpzpzpz(double p01, double p11, double p21, double D11,
								   double D21, double p02, double p12, double p22,
								   double D12, double D22, double R) {
		return qq(p01, p11, p21, D11, D21, p02, p12, p22, D12, D22, R) +
				qQzz(p01, p11, p21, D11, D21, p02, p12, p22, D12, D22, R) +
	...
	private static double spzppippi(double p01, double p11, double p21, double D11,
									double D21, double p02, double p12, double p22,
									double D12, double D22, double R) {
		return -quz(p02, p12, p22, D12, D22, p01, p11, p21, D11, D21, R) +
	...
	private static double spzpzpz(double p01, double p11, double p21, double D11,
								  double D21, double p02, double p12, double p22,
								  double D12, double D22, double R) {
		return -quz(p02, p12, p22, D12, D22, p01, p11, p21, D11, D21, R) +
	...
	private static double ppippispz(double p01, double p11, double p21, double D11,
									double D21, double p02, double p12, double p22,
									double D12, double D22, double R) {
		return quz(p01, p11, p21, D11, D21, p02, p12, p22, D12, D22, R) -
	...
	private static double pzpzspz(double p01, double p11, double p21, double D11,
								  double D21, double p02, double p12, double p22,
								  double D12, double D22, double R) {
		return quz(p01, p11, p21, D11, D21, p02, p12, p22, D12, D22, R) -
	...
	private static double sppisppi(double p01, double p11, double p21, double D11,
								   double D21, double p02, double p12, double p22,
								   double D12, double D22, double R) {
		return upiupi(p01, p11, p21, D11, D21, p02, p12, p22, D12, D22, R);
	}

	private static double spzspz(double p01, double p11, double p21, double D11,
								 double D21, double p02, double p12, double p22,
								 double D12, double D22, double R) {
		return uzuz(p01, p11, p21, D11, D21, p02, p12, p22, D12, D22, R);
	}

	private static double sppippipz(double p01, double p11, double p21, double D11,
									double D21, double p02, double p12, double p22,
									double D12, double D22, double R) {
		return upiQpiz(p01, p11, p21, D11, D21, p02, p12, p22, D12, D22, R);
	}

	private static double ppipzsppi(double p01, double p11, double p21, double D11,
									double D21, double p02, double p12, double p22,
									double D12, double D22, double R) {
		return -upiQpiz(p02, p12, p22, D12, D22, p01, p11, p21, D11, D21, R);
	}

	private static double ppipzppipz(double p01, double p11, double p21, double D11,
									 double D21, double p02, double p12, double p22,
									 double D12, double D22, double R) {
		return QpizQpiz(p01, p11, p21, D11, D21, p02, p12, p22, D12, D22, R);
	}

	private static double pxpypxpy(double p01, double p11, double p21, double D11,
								   double D21, double p02, double p12, double p22,
								   double D12, double D22, double R) {
						...
						switch (c.L) {
							...
							case 0: //(ss|s?);
								...
								switch (d.L) {
									...
									case 0://(ss|ss)
										return ssss(a.p0, a.p1, a.p2, a.D1, a.D2, c.p0,
												c.p1, c.p2, c.D1, c.D2, R);

									case 1:
										if (d.k == 1) {//(ss|spz)
											return ssspz(a.p0, a.p1, a.p2, a.D1, a.D2,
													c.p0, c.p1, c.p2, c.D1, c.D2, R);
										}
										else {//(ss|sppi) = 0
											return 0;
										}
							...
							case 1: //(ss|p?)
								if (c.k == 1) {//(ss|pz?)
									...
									switch (d.L) {
										...
										case 1:
											if (d.k == 1) {//(ss|pzpz)
												return sspzpz(a.p0, a.p1, a.p2, a.D1,
									...
									if (d.L == 1 && d.k == 0 && c.i == d.i &&
											c.j == d.j) {//(ss|ppippi)
										return ssppippi(a.p0, a.p1, a.p2, a.D1, a.D2,
												c.p0, c.p1, c.p2, c.D1, c.D2, R);
									}
						...
						if (b.k == 1) {//(spz|??)
							...
							switch (c.L) {
								...
								case 0://(spz|s?)
									...
									switch (d.L) {
										...
										case 0://(spz|ss)
											return spzss(a.p0, a.p1, a.p2, a.D1, a.D2,
													c.p0, c.p1, c.p2, c.D1, c.D2, R);

										case 1:
											if (d.k == 1) {//(spz|spz)
												return spzspz(a.p0, a.p1, a.p2, a.D1,
														a.D2, c.p0, c.p1, c.p2, c.D1,
														c.D2, R);
											}
											else {
								...
								case 1:
									if (c.k == 1) {//(spz|pz?)
										...
										if (d.i == c.i && d.j == c.j && d.k == 0) {
											return spzppippi(a.p0, a.p1, a.p2, a.D1,
													a.D2,
													c.p0, c.p1, c.p2, c.D1, c.D2, R);
										}
										else {
							...
							switch (c.L) {
								case 0://(sppi|s?)
									if (d.i == b.i && d.j == b.j &&
											d.k == 0) {//(sppi|sppi)
										return sppisppi(a.p0, a.p1, a.p2, a.D1, a.D2,
												c.p0, c.p1, c.p2, c.D1, c.D2, R);
									}
									else {
										return 0;
								...
								case 1:
									if (c.k == 1) {
										if (d.i == b.i && d.j == b.j &&
												d.k == 0) {//(sppi|pzppi)
											return sppippipz(a.p0, a.p1, a.p2, a.D1,
													a.D2,
													c.p0, c.p1, c.p2, c.D1, c.D2, R);
										}
										else {
										...
										if (c.i == b.i && c.j == b.j &&
											...
											switch (d.L) {
												case 0:
													return sppisppi(a.p0, a.p1, a.p2,
															a.D1, a.D2, c.p0, c.p1, c.p2,
															c.D1, c.D2, R);
												case 1:
													if (d.k == 1) {
														return sppippipz(a.p0, a.p1,
																a.p2,
																a.D1, a.D2, c.p0, c.p1,
																c.p2, c.D1, c.D2, R);
													}
							...
							if (b.k == 1) {//(pzpz|??)
								...
								switch (c.L) {
									...
									case 0://(pzpz|s?)
										...
										switch (d.L) {
											...
											case 0://(pzpz|ss)
												return pzpzss(a.p0, a.p1, a.p2, a.D1,
														a.D2, c.p0, c.p1, c.p2, c.D1,
														c.D2, R);

											case 1:
												if (d.k == 1) {//(pzpz|spz)
													return pzpzspz(a.p0, a.p1, a.p2,
															a.D1,
															a.D2, c.p0, c.p1, c.p2, c.D1,
															c.D2, R);
												}
									...
									case 1:
										if (c.k == 1) {//(pzpz|pz?)
											...
											switch (d.L) {
												...
												case 1:
													if (d.k == 1) {//(pzpz|pzpz)
														return pzpzpzpz(a.p0, a.p1, a.p2,
																a.D1, a.D2, c.p0, c.p1,
																c.p2, c.D1, c.D2, R);
													}
													else {//(pzpz|pzppi) = 0
											...
											if (d.i == c.i && d.j == c.j && d.k == 0) {
												return pzpzppippi(a.p0, a.p1, a.p2, a.D1,
														a.D2, c.p0, c.p1, c.p2, c.D1,
														c.D2, R);
											}
											else {
								...
								switch (c.L) {
									case 0://(pzppi|s?)
										if (d.i == b.i && d.j == b.j &&
												d.k == 0) {//(pzppi|sppi)
											return ppipzsppi(a.p0, a.p1, a.p2, a.D1,
													a.D2,
													c.p0, c.p1, c.p2, c.D1, c.D2, R);
										}
										else {
									...
									case 1:
										if (c.k == 1) {
											if (d.i == b.i && d.j == b.j &&
													d.k == 0) {//(pzppi|pzppi)
												return ppipzppipz(a.p0, a.p1, a.p2, a.D1,
														a.D2, c.p0, c.p1, c.p2, c.D1,
														c.D2, R);
											}
											else {
								...
								switch (c.L) {
									case 0://(ppippi|s?)
										switch (d.L) {
											case 0://(ppippi|ss)
												if (a.i == b.i && a.j == b.j) {
													return ppippiss(a.p0, a.p1, a.p2,
															a.D1, a.D2, c.p0, c.p1, c.p2,
															c.D1, c.D2, R);
												}
												else {
											...
											case 1:
												if (d.k == 1 && a.i == b.i &&
														a.j == b.j) {
													return ppippispz(a.p0, a.p1, a.p2,
															a.D1, a.D2, c.p0, c.p1, c.p2,
															c.D1, c.D2, R);
												}
												else {
									...
									case 1:
										if (c.k == 1) {
											...
											if (a.i == b.i &&
													a.j == b.j) {//(pxpx|??) or (pypy|??)

												if (c.L == d.L && c.i == d.i &&
														c.j == d.j && c.k == 0) {
													if (a.i == c.i) {
														return ppippippippi(a.p0, a.p1,
																a.p2, a.D1, a.D2, c.p0,
																c.p1, c.p2, c.D1, c.D2,
																R);
													}
													else {
														return pxpxpypy(a.p0, a.p1, a.p2,
																a.D1, a.D2, c.p0, c.p1,
																c.p2, c.D1, c.D2, R);
													}
												}
												...
												if (c.L == d.L && c.i != d.i &&
														c.j != d.j && c.k == 0) {
													return pxpypxpy(a.p0, a.p1, a.p2,
															a.D1, a.D2, c.p0, c.p1, c.p2,
															c.D1, c.D2, R);
												}
											}
