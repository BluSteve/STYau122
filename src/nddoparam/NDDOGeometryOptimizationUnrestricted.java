...
public class NDDOGeometryOptimizationUnrestricted extends NDDOGeometryOptimization {
	...
	protected double derivative(int i, int j) {
		return NDDODerivative.grad((NDDOSolutionUnrestricted) s, i, j);
	}

	protected DoubleMatrix[] routine() {
		DoubleMatrix[][] matrices =
				NDDODerivative.gradientroutine(atoms, (NDDOSolutionUnrestricted) s);

		DoubleMatrix gradient = matrices[0][0];

		DoubleMatrix hessian = NDDOSecondDerivative
				.hessianroutine(atoms, (NDDOSolutionUnrestricted) s, matrices[1],
						matrices[2]);
