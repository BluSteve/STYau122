...
public class ParamErrorFunctionUnrestricted extends ParamErrorFunction {
    ...
    protected double getGradient(int i, int j) {
        return NDDODerivative.grad((NDDOSolutionUnrestricted) expSoln, i, j);
    }

    @Override
    protected double getDeriv(double[] coeff, int atom2) {
        double deriv = coeff[0] * NDDODerivative.grad((NDDOSolutionUnrestricted) expSoln, atom2, 0)
                + coeff[1] * NDDODerivative.grad((NDDOSolutionUnrestricted) expSoln, atom2, 1)
                + coeff[2] * NDDODerivative.grad((NDDOSolutionUnrestricted) expSoln, atom2, 2);
