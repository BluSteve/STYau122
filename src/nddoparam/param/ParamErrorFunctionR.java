...
public class ParamErrorFunctionR extends ParamErrorFunction {
	...
	protected double getGradient(int i, int j) {
		return Derivative.grad((SolutionR) expSoln, i, j);
	}

	@Override
	protected double getDeriv(double[] coeff, int atom2) {
		double deriv =
				coeff[0] * Derivative.grad((SolutionR) expSoln, atom2
						, 0)
						+ coeff[1] *
						Derivative.grad((SolutionR) expSoln, atom2, 1)
						+ coeff[2] *
						Derivative.grad((SolutionR) expSoln, atom2, 2);
