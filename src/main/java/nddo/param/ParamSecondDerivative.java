...
public class ParamSecondDerivative {
	...
	public static double MNDOHFDeriv2(SolutionR soln, int Z1, int param1, int Z2, int param2, SimpleMatrix Hfirst,
	...
	public static double MNDOHFDeriv2(SolutionU soln, int Z1, int param1, int Z2, int param2, SimpleMatrix Hfirst,
	...
	public static SimpleMatrix staticMatrix(SolutionR soln, SimpleMatrix Fstatictotal, SimpleMatrix FockA,
											SimpleMatrix FockB, SimpleMatrix xA, SimpleMatrix xB, SimpleMatrix densityA, SimpleMatrix densityB, SimpleMatrix densityderiv2static,
											int Z1, int param1, int Z2, int param2) {
	...
	public static SimpleMatrix staticFockDeriv (SolutionR soln, SimpleMatrix Fstatictotal, SimpleMatrix densityA, SimpleMatrix densityB, SimpleMatrix densityderiv2static,
											int Z1, int param1, int Z2, int param2) {
	...
	public static SimpleMatrix[] staticMatrix(SolutionU soln, SimpleMatrix Fstatictotalalpha,
		...
		SimpleMatrix[] Ra = PopleThiel.responseMatrices(soln, densitiesA);

		SimpleMatrix FockAalpha = FstaticAalpha.plus(Ra[0]);
		SimpleMatrix FockAbeta = FstaticAbeta.plus(Ra[1]);

		SimpleMatrix[] Rb = PopleThiel.responseMatrices(soln, densitiesB);
		...
		SimpleMatrix[] omega = PopleThiel.responseMatrices(soln,
...
//		SimpleMatrix gammavec = PopleThiel.pople(soln, new SimpleMatrix[]{rhsmat})[0];
		...
		double Hfderivtest = ParamSecondDerivative.MNDOHFDeriv2(soln, Z1, param1, Z2, param2, H1, F1, D2, 0);

		double Hfderivtest2 = ParamSecondDerivative.MNDOHFDeriv2(soln, Z1, param1, Z2, param2, H2, F2, D1, 1);
		...
		SimpleMatrix x2matprime =
				ParamDerivative.xMatrix(solnprime,
						solnprime.Ct.mult(F2prime.plus(PopleThiel.responseMatrix(solnprime, D2prime)))
						.mult(solnprime.C));
		...
		double IEderiv = ParamDerivative.homoDeriv(soln, x2mat, Fderivb);

		double IEderivprime = ParamDerivative.homoDeriv(solnprime, x2matprime, Fderivbprime);
	...
	public static boolean verifyEquations(SolutionU soln, int Z1, int param1, int Z2, int param2) {
		...
		SimpleMatrix[] x1matrix = ParamDerivative.xMatrix(soln, soln.Cta.mult(F1alpha.plus(R1[0])).mult(soln.Cta.transpose()),
		...
		SimpleMatrix[] R2 = PopleThiel.responseMatrices(soln, D2);

		SimpleMatrix[] R2prime = PopleThiel.responseMatrices(solnprime, D2prime);


		SimpleMatrix[] x2matrix = ParamDerivative.xMatrix(soln, soln.Cta.mult(F2alpha.plus(R2[0])).mult(soln.Cta.transpose()),
				soln.Ctb.mult(F2beta.plus(R2[1])).mult(soln.Ctb.transpose()));

		SimpleMatrix[] x2primematrix =
				ParamDerivative.xMatrix(solnprime, solnprime.Cta.mult(F2alphaprime.plus(R2prime[0])).mult(solnprime.Cta.transpose()),
		...
		double Hfderivtest =
				ParamSecondDerivative.MNDOHFDeriv2(soln, Z1, param1, Z2, param2, H1, F1alpha, F1beta, D2[0], D2[1], 0);

		double Hfderivtest2 =
				ParamSecondDerivative.MNDOHFDeriv2(soln, Z1, param1, Z2, param2, H2, F2alpha, F2beta, D1[0], D1[1], 1);

		SimpleMatrix totalderiv = rhsmat[0].plus(
				soln.Cta.mult(PopleThiel.responseMatrices(soln, densityderiv2response)[0])
		...
		SimpleMatrix Fderiv2 =
				staticFockDeriv(soln, Fstatictotalalpha, Fstatictotalbeta, F1alpha, F1beta, F2alpha, F2beta, x1, x2,
						Z1,
						param1, Z2, param2)[0].plus(PopleThiel.responseMatrices(soln, densityderiv2response)[0]);
...
//
		...
		double IEderiv = ParamDerivative.homoDeriv(soln, x2matrix[0], Fderivb);
		double IEderivprime =
				ParamDerivative.homoDeriv(solnprime, x2primematrix[0], Fderivbprime);
		double IEderiv2 = 1 / Constants.LAMBDA * (IEderivprime - IEderiv);
//
		System.err.println("----TESTING IE DERIVATIVES----");
//
...
//
	...
	public static boolean verifyEquations(SolutionU soln, int Z1, int param1) {
		...
		SimpleMatrix[] responsematrices = PopleThiel.responseMatrices(soln, densityderivs);
